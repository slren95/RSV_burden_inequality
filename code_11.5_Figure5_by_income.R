#
# ------------------------------------------------------------------------------
# Script: code_11.5_Figure5_by_income.R
# Purpose:
#   Create Figure 5-style visualisations summarising projected reductions in
#   RSV-associated hospitalisations and deaths, stratified by income level.
#
# Inputs:
...
#
# Outputs:
#   - Figure files saved via `ggsave()` / chart exports (see script body).
#
# Usage:
#   source("code_11.5_Figure5_by_income.R")
# ------------------------------------------------------------------------------
...
# Figure 5 by Income----
getFigure5_by_income <- function(sum_by_income,df_res.main,barwidth=0.3,showlabel=FALSE) {
  labels <- if_else(rep(showlabel,3),
                    c('2019','2026 \n (no implementation)','2026 \n (status quo implementation)'),
                    rep('',3))
  # replace
  sum_by_region<-sum_by_income %>%
    rename(WHORegion=Income2019)
  ...
  data_23<-df_plot5 %>%
    ...
    mutate(WHORegion=factor(WHORegion,levels=c('LM','UM','H','L'))) %>%
    arrange(N,WHORegion) %>%
    group_by(N) %>%
  ...
  df_percent_23<-sum_by_income %>%
    rename(WHORegion=Income2019) %>% # Replace
  ...
  data_23<-df_plot5 %>%
    ...
    mutate(WHORegion=factor(WHORegion,levels=c('LM','L','UM','H'))) %>%
    arrange(N,WHORegion) %>%
    group_by(N) %>%
    mutate(cum=cumsum(value)) %>%
    pivot_wider(id_cols = WHORegion,names_from = N,values_from = cum) %>%
  ...
  p52<-df_plot5 %>%
    ...
    scale_fill_manual(name=NULL,
                      values=c("L" = "#1b9e77",        # 深绿色 - 低收入
                               ...
                               "UM" = "#e6ab02",       # 橙黄 - 中高收入
                               "H" = "#d95f02"),        # 深橙红 - 高收入
                      labels = c("L"="LIC", "LM" = "LMIC","UM" = "UMIC","H" = "HIC"),
                      breaks = c('H','UM','LM','L')
    )+
...
getFigure5_by_income(sum_by_income,df_res.main)

ggsave('pdf/Figure5_by_income.pdf',width = 13,height = 8)
...
df_res.all %>% 
  ...
  iwalk(~{
    ...
    sum_by_income_with_global<-bind_rows(
      df_res.main %>% add_by(Income2019),
      df_res.main %>% mutate(Income2019='Global') %>% add_by(Income2019)
    ) %>% 
      sum_by(Income2019) %>%
      rename(pop_infant_2019=pop_infant_2019_q500,
             pop_infant_2026=pop_infant_2026_q500) %>%
    ...
    sum_by_income_with_global %>%
      ...
      arrange(Income) %>%
      `[`(c(2,5,4,3,1),) %>%
      createAppendixTable(file_path=sprintf('docs/Model_Result_Main_by_income_%s.xlsx',params))
    
    getFigure5_by_income(sum_by_income,df_res.main)
    ggsave(filename = sprintf('pdf/Figure5_by_income_%s.pdf',params),width = 13,height = 8)
  })

# Plot List 
plots_by_income<-df_res.all %>% 
  group_split(params) %>% 
  map(~{
    df_res.main<-.x
    params=.x[[1,'params_str']] %>% str_replace_all('[, ]','_')
    ...
    getFigure5_by_income(sum_by_income,df_res.main,showlabel = TRUE)
  })

# plots_by_region 
# main # uptake 0.9 0.3 # uptake 0.7 0.1 # eff 0.88 # eff 0.78
plots_by_region<-import('rda/plots_by_region.rds')

scenarios=c('main','uptake_0.9','uptake_0.7','eff_0.88','eff_0.78')
  
1:5 %>%
  walk(~{
    plots_by_region[[.x]]/ plots_by_income[[.x]]+
      plot_layout(axis_titles = "collect") +
      plot_annotation( tag_levels = 'A')+
      theme(
        plot.tag = element_text(size = 20, face = "bold")  # 这里调大小、加粗、颜色
      )
    ggsave(sprintf('pdf/Figure5_sensitive_%s.pdf',scenarios[.x]),width = 14,height = 15)
  })
  

plots_by_region[[1]]/plots_by_income[[1]]+plot_layout(nrow=2,axis_titles = "collect_y")+
  plot_annotation(tag_levels = 'A')
